...
// Define a minimal interface for the Jodit editor instance,
// including only the properties you need (e.g. i18n)
interface MinimalJodit {
  i18n: (key: string, ...args: unknown[]) => string;
}

const linkFormOverride = (editor: MinimalJodit) => {
  const i18n = editor.i18n.bind(editor);

  return `<form class="jodit_form">
  <div class="${styles.jodit_form_group}">
    <label class="${styles["jodit-input-label"]}" for="url">URL</label>
    <input ref="url_input" class="${styles.jodit_input}" required type="text" id="url" name="url" placeholder="http://" />
  </div>
  <div ref="content_input_box" class="${styles.jodit_form_group}">
    <label class="${styles["jodit-input-label"]}" for="text">Text</label>
    <input ref="content_input" class="${styles.jodit_input}" id="text" required name="text" placeholder="${i18n("Text")}" type="text"/>
  </div>
  <label ref="target_checkbox_box">
    <input ref="target_checkbox" class="jodit_checkbox" name="target" type="checkbox" checked/>
    <span>${i18n("Open in new tab")}</span>
  ...
  <div class="${styles.jodit_buttons}">
    <button ref="unlink" class="${styles.jodit_button} ${styles.jodit_unlink_button}" type="button">${i18n("Unlink")}</button>
    <button ref="insert" class="${styles.jodit_button} ${styles.jodit_link_insert_button}" type="submit">${i18n("Insert")}</button>
  </div>
  </form>`;
};
